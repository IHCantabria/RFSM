...
def export_result_RFSM(path_results,TestDesc,topography,izd2_r,Results,src):
    """La siguiente función permite transformar los ficheros de SLR de Matlab en ficheros NETCDF

    Parámetros:
    ---------------------
    path_results           : string. Directorio donde se encuentra los resultados de RFSM
    ...
    path_shp_iz2  = join_string(izd2_r.split('/')[:-1])
    path_tif_topo = join_string(topography.split('/')[:-1])
    
    if os.path.exists(path_shp_iz2+'/izid2.shp')==False:
        print('######## Creando shapefile izid2 #########')

        in_path = izd2_r

        out_path = path_shp_iz2+'/izid2.shp'
        ...
        sp_ref.SetFromUserInput('EPSG:'+str(src))

        dst_layer = dst_ds.CreateLayer(dst_layername, srs = sp_ref )

        fld = ogr.FieldDefn("IZID", ogr.OFTInteger)
        dst_layer.CreateField(fld)
        dst_field = dst_layer.GetLayerDefn().GetFieldIndex("IZID")
        mask = src_ds.GetRasterBand(1).GetMaskBand()

        gdal.Polygonize( srcband, mask, dst_layer, dst_field, [], callback=None )
        src_ds    = None
        mask      = None
        dst_layer = None 
        dst_field = None
        ...
        #gdal_polygonize(izd2_r+' '+path_shp_iz2+'/izid2.shp -b 1 -f "ESRI Shapefile" izid2 IZID')
    
    path_results_2 = path_results + 'tests/'+TestDesc+'/Results_'+str(Results)+'/'
    tusrResultsIZMax = pd.read_csv(path_results_2+'tusrResultsIZMax.csv',index_col=1)
    shape_IZID2 = gpd.read_file(path_shp_iz2+'/izid2.shp')
    shape_IZID2['Level'] = np.nan
    
    if os.path.exists(path_tif_topo+'/topography.tif')==False:
        print('######## Creando fichero tif Topography #########')
        ds = gdal.Open(topography)
        ds = gdal.Translate(path_tif_topo+'/topography.tif', ds)
        ds = None
        gdal_edit((" -stats -a_srs EPSG:"+str(src)+" -a_nodata -9999 " +path_tif_topo+'/topography.tif').split(" "))
    ...
    shape_IZID2.to_file(path_results_2+'/izid2_Level.shp')
    print('Crando Raster Maxlevel')
    rasterize (path_results_2+'/izid2_Level.shp',
               'Level',
               path_tif_topo+'/topography.tif',
               path_results + 'tests/'+TestDesc+'/export/Result_Level_IZID'+str(Results)+'.tif' )
               
    gdal_edit((" -stats -a_srs EPSG:"+str(src)+" -a_nodata -9999 " +  path_results + 'tests/'+TestDesc+'/export/Result_Level_IZID'+str(Results)+'.tif').split(" "))
    
    A   =  path_tif_topo  + '/topography.tif'
    B   =  path_results + 'tests/'+TestDesc + '/export/Result_Level_IZID'+str(Results)+'.tif' 
    R   =  path_results + 'tests/'+TestDesc + '/export/MaxLevel_'+TestDesc+'_init.tif'
    R_2 =  path_results + 'tests/'+TestDesc + '/export/MaxLevel_'+TestDesc+'.tif'
    ...
    gdal_edit((" -stats -a_srs EPSG:"+str(src)+" -a_nodata -9999 " +  path_results + 'tests/'+TestDesc+'/export/MaxLevel_'+TestDesc+'.tif').split(" "))
